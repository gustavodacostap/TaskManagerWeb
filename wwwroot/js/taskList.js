// Responsável por renderizar, atualizar, editar, excluir e completar tarefas na interface

import { formState } from "./formState.js";

let taskToDelete = null; // Armazena a tarefa que será excluída após confirmação no modal

// Carrega as tarefas da API e renderiza na interface
...
// Renderiza todas as tarefas em uma <ul>
...
// Cria o HTML de cada item da tarefa e adiciona eventos (editar, excluir, checkbox)
function renderTaskItem(task) {
    ...
    <div class="form-check d-flex align-items-center gap-2" style="min-height: 38px;">
      <input class="form-check-input mt-0 rounded-circle cursor-pointer" type="checkbox"
    ...
    <div class="d-md-flex gap-2 position-absolute end-0 me-3 task-actions">
  ...
  `;
    ...
    li.querySelector("input[type='checkbox']").addEventListener("change", (e) => {
        ...
        // Estiliza a label com riscado se a tarefa estiver concluída
...
// Entra no modo de edição da tarefa (substitui <li> por um <form>)
function enterEditMode(li, task) {
    // Caso já haja outro formulário aberto (criação ou edição), reseta
    if ((formState.type === "create" || formState.type === "edit") && typeof formState.reset === "function") {
        formState.reset();
    }
  ...
  `;
    ...
    // Atualiza o estado global para "edit"   
    formState.type = "edit";
    formState.reset = () => {
...
// Configura o botão de excluir e mostra o modal de confirmação
...
// Evento do botão de confirmação de exclusão (fora do escopo do botão delete)
